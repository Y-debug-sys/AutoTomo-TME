...
class Trainer(object):
    def __init__(self, model, rm, data_loader, results_folder='./check_points', train_lr=1e-4, known_id=[],
                 train_epoch=1000, batch_size=64, patience=100, loss_type='mse', model_name='bpnn'):
        super().__init__()
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.model = model.to(self.device)
        self.rm, self.known_id = rm.to(self.device), known_id
        ...
        self.early_stopping = EarlyStopping(patience=patience, path=os.path.join(results_folder, f'{model_name}_best.pt'),verbose=False)
        self.step = 0
        self.counter = 50
    ...
    def load(self, mode='best'):
        assert mode in ['best', 'last'], 'Sorry, there is only the best checkpoint and last checkpoint saved during training!'
        if mode == 'best':
            self.model.load_state_dict(torch.load(os.path.join(self.results_folder, f'{self.model_name}_best.pt')))
        else:
            self.model.load_state_dict(torch.load(os.path.join(self.results_folder, f'{self.model_name}_last.pt')))
    ...
    def train(self):
        ...
        print('Training Complete')
        torch.save(self.model.state_dict(), os.path.join(self.results_folder, f'{self.model_name}_last.pt'))
        self.load('best')
    ...
    def estimate(self, data_loader):
        ...
        for idx, (x, y) in enumerate(data_loader):
            x, y = x.to(self.device), y.to(self.device)
            y_hat = self.model(x)
            estimations = np.row_stack([estimations, y_hat.cpu().numpy()])
            test_loss_y = self.criteon(y_hat, y)
            test_loss.append(test_loss_y.item())
                # counter_loss += test_loss_y.item() / self.counter
                # if self.step % self.counter == 0:
                #     pbar.set_description(f'Error: {test_loss_y.item():.6f}')
